...
# サブモニターを使用（存在する場合）
if len(monitors) > 1:
    monitor = monitors[2] if len(monitors) > 2 else monitors[1]  # サブモニター
    print("サブモニターを使用します")
else:
...
show_red_grid = True  # 赤い9分割グリッドの表示制御
...
def create_red_grid_lines(face_vertices):
    """赤い3x3グリッド線を生成"""
    grid_vertices = []
    grid_colors = []
    red_color = [0.0, 1.0, 0.0]  # 赤色
    
    divisions = 3  # 3x3 = 9分割
    
    # 面の4つの頂点から補間してグリッド点を生成
    grid_points = []
    ...
    for i in range(divisions + 1):
        for j in range(divisions):
            ...
            grid_vertices.extend([grid_points[idx1], grid_points[idx2]])
            grid_colors.extend([red_color, red_color])
...
def create_checkerboard_pattern(face_vertices, divisions):
    """チェッカーボードパターンを生成"""
    vertices = []
    colors = []
    ...
    return face_vertices_list, face_colors_list
...
def create_test_pattern():
    ...
    all_grid_vertices = []
    all_grid_colors = []
    all_red_grid_vertices = []  # 赤いグリッド線用
    all_red_grid_colors = []
    ...
    for i, face_verts in enumerate(faces):
        ...
        # チェッカーボードパターン
        if pattern_mode == 1:
            face_verts_list, face_colors_list = create_checkerboard_pattern(face_verts, grid_divisions)
            grid_verts, grid_colors_list = [], []
        # フォトニックフラクタルパターン
        elif pattern_mode == 4:
        ...
        # 赤い3x3グリッド線を生成
        if show_red_grid:
            red_grid_verts, red_grid_colors = create_red_grid_lines(face_verts)
            all_red_grid_vertices.extend(red_grid_verts)
            all_red_grid_colors.extend(red_grid_colors)
    
    return all_vertices, all_colors, all_grid_vertices, all_grid_colors, all_red_grid_vertices, all_red_grid_colors
...
# 赤いグリッド用のVAO/VBO
red_grid_VAO = glGenVertexArrays(1)
red_grid_VBO = glGenBuffers(1)
red_grid_color_VBO = glGenBuffers(1)

def update_buffers():
    """バッファを更新"""
    face_vertices, face_colors, grid_vertices, grid_colors, red_grid_vertices, red_grid_colors = create_test_pattern()
    ...
    # 赤いグリッドのバッファ更新
    if red_grid_vertices:
        glBindVertexArray(red_grid_VAO)
        
        red_grid_vertices_np = np.array(red_grid_vertices, dtype=np.float32)
        glBindBuffer(GL_ARRAY_BUFFER, red_grid_VBO)
        glBufferData(GL_ARRAY_BUFFER, red_grid_vertices_np.nbytes, red_grid_vertices_np, GL_DYNAMIC_DRAW)
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, None)
        glEnableVertexAttribArray(0)
        
        red_grid_colors_np = np.array(red_grid_colors, dtype=np.float32)
        glBindBuffer(GL_ARRAY_BUFFER, red_grid_color_VBO)
        glBufferData(GL_ARRAY_BUFFER, red_grid_colors_np.nbytes, red_grid_colors_np, GL_DYNAMIC_DRAW)
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, None)
        glEnableVertexAttribArray(1)
    
    return len(face_vertices), len(grid_vertices), len(red_grid_vertices)
...
glLineWidth(2.0)  # 線の太さを設定（赤いグリッド線を太くする）
...
while not glfw.window_should_close(window):
    ...
    if glfw.get_key(window, glfw.KEY_R) == glfw.PRESS:
        show_red_grid = not show_red_grid
        time.sleep(0.1)  # キーリピート防止
    ...
    glClearColor(0.0, 0.0, 0.1, 1.0)  # 暗い青背景（フォトニック効果を強調）
    
    # バッファ更新
    face_count, grid_count, red_grid_count = update_buffers()
    ...
    if show_edges and grid_count > 0:
        glLineWidth(1.0)  # 通常のグリッド線は細く
        glBindVertexArray(grid_VAO)
        glDrawArrays(GL_LINES, 0, grid_count)
    
    # 赤い3x3グリッド線を描画
    if show_red_grid and red_grid_count > 0:
        glLineWidth(15.0)  # 赤いグリッド線は太く
        glBindVertexArray(red_grid_VAO)
        glDrawArrays(GL_LINES, 0, red_grid_count)
    ...
    if int(animation_time) % 2 == 0:  # 2秒おきに表示
        ...
        status += f", グリッド: {'ON' if show_edges else 'OFF'}, 赤グリッド: {'ON' if show_red_grid else 'OFF'}"
        print(status, end="")
