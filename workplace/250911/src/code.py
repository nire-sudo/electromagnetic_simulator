...
def update_particles_optimized(particles, E_field_array):
    ...
    cuda_instance_data = mapping.device_ptr()  # mappingから直接デバイスポインタを取得
    ...
    cuda_func(
        ...
        gpu_arrays['instance'],  # VBOへの直接ポインタ
    ...
    instance_data = np.zeros(N * 7, dtype=np.float32)
    cuda.memcpy_dtoh(instance_data, gpu_arrays['instance'])
...
while not glfw.window_should_close(window):
    ...
    if gui_params['show_field_vectors']:
        ...
        if len(field_vectors) > 0:
            ...
            glUniform3fv(vector_light_pos_loc, 1, np.array([5.0, 5.0, 5.0], dtype=np.float32))
