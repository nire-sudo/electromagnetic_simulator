...
extern "C" __global__ void update_particles_cuda(
    ...
    if(geometry_mode == 0) {
        // X軸境界処理
        if (pi.x < -half_box) {
            if (cube_vanish_flags[0]) {
                // X負側で消滅
                vanish_flags[i] = 1;
                return;
            } else if (warp_flags[0]) {
                // X負側ワープ
                pi.x = half_box - 0.01f;
            } else {
                // X負側跳ね返り
            ...
            if (cube_vanish_flags[1]) {
                // X正側で消滅
                vanish_flags[i] = 1;
                return;
            } else if (warp_flags[1]) {
                // X正側ワープ
                pi.x = -half_box + 0.01f;
            } else {
                // X正側跳ね返り
        ...
        // Y軸境界処理
        if (pi.y < -half_box) {
            if (cube_vanish_flags[2]) {
                // Y負側で消滅
                vanish_flags[i] = 1;
                return;
            } else if (warp_flags[2]) {
                // Y負側ワープ
                pi.y = half_box - 0.01f;
            } else {
                // Y負側跳ね返り
            ...
            if (cube_vanish_flags[3]) {
                // Y正側で消滅
                vanish_flags[i] = 1;
                return;
            } else if (warp_flags[3]) {
                // Y正側ワープ
                pi.y = -half_box + 0.01f;
            } else {
                // Y正側跳ね返り
        ...
        // Z軸境界処理
        if (pi.z < -half_box) {
            if (cube_vanish_flags[4]) {
                // Z負側で消滅
                vanish_flags[i] = 1;
                return;
            } else if (warp_flags[4]) {
                // Z負側ワープ
                pi.z = half_box - 0.01f;
            } else {
                // Z負側跳ね返り
            ...
            if (cube_vanish_flags[5]) {
                // Z正側で消滅
                vanish_flags[i] = 1;
                return;
            } else if (warp_flags[5]) {
                // Z正側ワープ
                pi.z = -half_box + 0.01f;
            } else {
                // Z正側跳ね返り
    ...
    if (vanish_flags[i] == 0) {  // 生存している粒子のみ
        ...
        // 消滅した粒子はスケールを0にして非表示
        instance_vbo[vbo_offset + 0] = 0.0f;
        instance_vbo[vbo_offset + 1] = 0.0f;
        instance_vbo[vbo_offset + 2] = 0.0f;
        instance_vbo[vbo_offset + 3] = 0.0f;  // スケール0で非表示
        instance_vbo[vbo_offset + 4] = 0.0f;
        instance_vbo[vbo_offset + 5] = 0.0f;
        instance_vbo[vbo_offset + 6] = 0.0f;
    }
}

extern "C" __global__ void extract_contour_cuda(
    const float* grid_slice,
    float level,
    int width,
    int height,
    float* vertex_buffer,
    ...
    float v00 = grid_slice[i * width + j];
    float v10 = grid_slice[i * width + (j + 1)];
    float v01 = grid_slice[(i + 1) * width + j];
    float v11 = grid_slice[(i + 1) * width + (j + 1)];

    int case_idx = 0;
    if (v00 >= level) case_idx |= 1;
    if (v10 >= level) case_idx |= 2;
    if (v11 >= level) case_idx |= 4;
    if (v01 >= level) case_idx |= 8;

    if (case_idx == 0 || case_idx == 15) return;

    float edges[4][2];
    int edge_count = 0;

    if ((case_idx & 1) != (case_idx & 2)){
        float t = (level - v00) / (v10 - v00 + 1e-10f);
        t = fminf(fmaxf(t, 0.0f), 1.0f);
        edges[edge_count][0] = j + t;
        edges[edge_count][1] = i;
        edge_count++;
    }

    if ((case_idx & 2) != (case_idx & 4)){
        float t = (level - v10) / (v11 - v10 + 1e-10f);
        t = fminf(fmaxf(t, 0.0f), 1.0f);
        edges[edge_count][0] = j + 1;
        edges[edge_count][1] = i + t;
        edge_count++;
    }

    if ((case_idx & 8) != (case_idx & 4)){
        float t = (level - v01) / (v11 - v01 + 1e-10f);
        t = fminf(fmaxf(t, 0.0f), 1.0f);
        edges[edge_count][0] = j + t;
        edges[edge_count][1] = i + 1;
        edge_count++;
    }

    if ((case_idx & 1) != (case_idx & 8)){
        float t = (level - v00) / (v01 - v00 + 1e-10f);
        t = fminf(fmaxf(t, 0.0f), 1.0f);
        edges[edge_count][0] = j;
        edges[edge_count][1] = i + t;
        edge_count++;
    }

    if (edge_count >= 2){
        int base_idx = atomicAdd(vertex_count, 2);
        if (base_idx + 1 >= max_vertices) return;

        float x1, y1, z1, x2, y2, z2;

        if (plane_type == 0){
            x1 = (edges[0][0] / width - 0.5f) * box_size;
            z1 = (edges[0][1] / height - 0.5f) * box_size;
            y1 = slice_pos;
            x2 = (edges[1][0] / width - 0.5f) * box_size;
            z2 = (edges[1][1] / height - 0.5f) * box_size;
            y2 = slice_pos;
        } else if (plane_type == 1){
            x1 = (edges[0][0] / width - 0.5f) * box_size;
            y1 = (edges[0][1] / height - 0.5f) * box_size;
            z1 = slice_pos;
            x2 = (edges[1][0] / width - 0.5f) * box_size;
            y2 = (edges[1][1] / height - 0.5f) * box_size;
            z2 = slice_pos;
        } else{
            z1 = (edges[0][0] / width - 0.5f) * box_size;
            y1 = (edges[0][1] / height - 0.5f) * box_size;
            x1 = slice_pos;
            z2 = (edges[1][0] / width - 0.5f) * box_size;
            y2 = (edges[1][1] / height - 0.5f) * box_size;
            x2 = slice_pos;
        }

        int offset1 = base_idx * 6;
        vertex_buffer[offset1 + 0] = x1;
        vertex_buffer[offset1 + 1] = y1;
        vertex_buffer[offset1 + 2] = z1;
        vertex_buffer[offset1 + 3] = r;
        vertex_buffer[offset1 + 4] = g;
        vertex_buffer[offset1 + 5] = b;

        int offset2 = (base_idx + 1) * 6;
        vertex_buffer[offset2 + 0] = x2;
        vertex_buffer[offset2 + 1] = y2;
        vertex_buffer[offset2 + 2] = z2;
        vertex_buffer[offset2 + 3] = r;
        vertex_buffer[offset2 + 4] = g;
        vertex_buffer[offset2 + 5] = b;
    }
}
