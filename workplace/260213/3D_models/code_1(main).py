...
def update_particles_optimized(particles, E_field_array):
    ...
    E_field_flat = E_field_array.reshape(-1, 3).astype(np.float32) #電場データを一時配列に変換
    ...
    gpu_arrays = update_particles_optimized.gpu_arrays # GPUメモリへの参照を取得
    ...
    block_size = 512 # CUDAブロック内のスレッド数
    grid_size_cuda = (N + block_size - 1) // block_size # 必要なブロック数
...
def emit_particle():
    ...
    direction_norm = np.linalg.norm(direction) # 方向ベクトルの長さを計算
    if direction_norm > 0:
        # 正常な方向ベクトルの場合、単位ベクトルに正規化
        direction = direction / direction_norm
    else:
        # ゼロベクトルの場合、デフォルトでY軸正方向を設定
...
while not glfw.window_should_close(window):
    ...
    if imgui.collapsing_header("Magnetic Field Settings")[0]:
        ...
        if gui_params['enable_magnetic_field']:
            ...
            changed, gui_params['magnetic_field_x'] = imgui.slider_float(
                "B_x", gui_params['magnetic_field_x'], -20.0, 20.0
            )
            
            changed, gui_params['magnetic_field_y'] = imgui.slider_float(
                "B_y", gui_params['magnetic_field_y'], -20.0, 20.0
            )
            
            changed, gui_params['magnetic_field_z'] = imgui.slider_float(
                "B_z", gui_params['magnetic_field_z'], -20.0, 20.0
            )
    ...
    if imgui.collapsing_header("High Current Beam Presets")[0]:
        imgui.text("High current beam simulation presets")
        imgui.separator()
        
        if imgui.button("Space Charge Dominated Beam"):
            gui_params.update({
                'num_particles': 8000,  # 高密度
                'use_poisson': True,    # 空間電荷効果必須
                'enable_emitter': True,
                'emitter_position': [0.0, -1.9, 0.0],
                'emitter_direction': [0.0, 1.0, 0.0],
                'emitter_speed': 15.0,  # 高速
                'emitter_spread': 0.05,  # 細いビーム
                'emitter_rate': 1,      # 連続放射
                'emitter_charge_type': 0,  # 単極性（正電荷）
                'max_emitted_particles': 8000,
                'positive_charge': 2.0,  # 高電荷
                'negative_charge': -0.1,
                'particle_mass': 1.0,
                'dt': 0.001,            # 小さいタイムステップ
                'coulomb_force_control': 1.0,  # フル斥力
                'enable_magnetic_field': False,
                'external_E_field_x': 0.0,
                'external_E_field_y': 0.0,
                'external_E_field_z': 0.0,
                'electrode_mode': False,
                'geometry_mode': 1,     # 円筒形状
                'cylinder_top_warp': True,    # 上下でワープ
                'cylinder_bottom_warp': True,
                'cylinder_wall_vanish': True,  # 壁で消失
                'scale_factor': 0.02,
                'warp_x_negative': False,
                'warp_x_positive': False,
                'warp_y_negative': False,
            ...
            particles.clear()
            clear_gpu_arrays()
            print("Space charge dominated beam preset applied")
            print("High charge density will cause beam expansion due to space charge")
        
        if imgui.button("Magnetically Focused Beam"):
            gui_params.update({
                'num_particles': 6000,
                'use_poisson': True,
                'enable_emitter': True,
                ...
                'emitter_direction': [0.0, 1.0, 0.0],
                'emitter_speed': 20.0,
                'emitter_spread': 0.1,   # やや広めの初期拡散
                'emitter_rate': 2,
                'emitter_charge_type': 1,  # 負電荷（電子ビーム模擬）
                'max_emitted_particles': 6000,
                'positive_charge': 0.1,
                'negative_charge': -1.5,
                'particle_mass': 1.0,
                'dt': 0.0008,
                'coulomb_force_control': 0.8,  # やや弱い斥力
                'enable_magnetic_field': True,
                'magnetic_field_x': 0.0,
                'magnetic_field_y': 5.0,     # 軸方向磁場でピンチ効果
            ...
            particles.clear()
            clear_gpu_arrays()
            print("Magnetically focused beam preset applied")
            print("Axial magnetic field provides beam focusing")
        
        if imgui.button("Pierce Gun Configuration"):
            gui_params.update({
                'num_particles': 5000,
                'use_poisson': True,
                'enable_emitter': True,
                'emitter_position': [0.0, -1.9, 0.0],
                'emitter_direction': [0.0, 1.0, 0.0],
                'emitter_speed': 8.0,    # 低初速（加速前）
                'emitter_spread': 0.02,   # 非常に細いビーム
                'emitter_rate': 3,
                'emitter_charge_type': 1,  # 負電荷
                'max_emitted_particles': 5000,
                'positive_charge': 0.1,
                'negative_charge': -1.0,
                'particle_mass': 1.0,
                ...
                'coulomb_force_control': 1.0,
                'enable_magnetic_field': False,
                'electrode_mode': True,   # 加速電極使用
                'electrode_voltage': 50.0,  # 高加速電圧
                'electrode_positions': {
                    'x_neg': False, 'x_pos': False,
                    'y_neg': False, 'y_pos': True,  # 上部に加速電極
                    'z_neg': False, 'z_pos': False
                },
                'external_E_field_x': 0.0,
                'external_E_field_y': 0.0,
                'external_E_field_z': 0.0,
                'geometry_mode': 1,
                'cylinder_top_warp': True,
                'cylinder_bottom_warp': False,  # 下部は反射（カソード）
                'cylinder_wall_vanish': True,
                'scale_factor': 0.018
            })
            particles.clear()
            clear_gpu_arrays()
            print("Pierce gun configuration applied")
            print("Simulates electron gun with accelerating electrode")
        
        if imgui.button("Beam Transport Channel"):
            gui_params.update({
                'num_particles': 7000,
                'use_poisson': True,
                'enable_emitter': True,
                'emitter_position': [-1.8, 0.0, 0.0],  # 横から入射
                'emitter_direction': [1.0, 0.0, 0.0],   # X方向に進行
                'emitter_speed': 25.0,   # 高速
                'emitter_spread': 0.03,
                'emitter_rate': 1,
                'emitter_charge_type': 0,  # 正電荷
                'max_emitted_particles': 7000,
                'positive_charge': 1.5,
                'negative_charge': -0.1,
                'particle_mass': 2.0,    # 重粒子
                'dt': 0.0005,
                'coulomb_force_control': 0.9,
                'enable_magnetic_field': True,
                'magnetic_field_x': 0.0,
                'magnetic_field_y': 0.0,
                'magnetic_field_z': 3.0,  # ガイド磁場
                'external_E_field_x': 0.0,
                'external_E_field_y': 0.0,
                'external_E_field_z': 0.0,
                'electrode_mode': False,
                'geometry_mode': 0,      # 直方体チャンバー
                'warp_x_negative': True,  # X-でワープ（入口）
                'warp_x_positive': True,  # X+でワープ（出口）
                'warp_y_negative': False,
                'warp_y_positive': False,
                'warp_z_negative': False,
                'warp_z_positive': False,
            ...
            particles.clear()
            clear_gpu_arrays()
            print("Beam transport channel preset applied")
            print("Simulates beam propagation with guiding magnetic field")
        
        if imgui.button("Plasma Simulation"):
            gui_params.update({
                'num_particles': 8000,
                'use_poisson': True,
                'enable_emitter': False,  # 初期分布のみ
                'positive_charge': 1.0,
                'negative_charge': -1.0,  # 準中性プラズマ
                'particle_mass': 1.0,
                'dt': 0.0005,
                'coulomb_force_control': 1.0,
                'enable_magnetic_field': True,
                'magnetic_field_x': 0.0,
                'magnetic_field_y': 0.0,
                'magnetic_field_z': 8.0,   # 強い閉じ込め磁場
                'external_E_field_x': 0.0,
                'external_E_field_y': 0.0,
                'external_E_field_z': 0.0,
                'electrode_mode': False,
                'geometry_mode': 1,       # 円筒プラズマ
                'cylinder_top_warp': False,   # 完全閉じ込め
                'cylinder_bottom_warp': False,
                'cylinder_wall_vanish': False,
                'warp_x_negative': False,
                'warp_x_positive': False,
            ...
            particles = create_particles(gui_params['num_particles'])
            clear_gpu_arrays()
            print("Plasma simulation preset applied")
            print("Equal numbers of positive and negative charges in magnetic confinement")
    
        imgui.separator()
        imgui.text("Tips for high current beam simulation:")
        imgui.text("• Use Poisson equation for space charge effects")
        imgui.text("• Reduce time step for numerical stability")
        imgui.text("• Monitor beam envelope expansion")
        imgui.text("• Adjust magnetic field for focusing")
        imgui.text("• Consider particle mass effects")
